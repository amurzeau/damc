...
static size_t filter_farrow_resample_putfloat(
    struct filter_farrow_t* filter, float input, float ratio, float x0, float* output, int stride) {
	filter->history[filter->currentIndex] = input;
	filter->currentIndex = (filter->currentIndex + 1) & 0x3;
