...
std::complex<double> BiquadFilter::getResponse(double f0, double fs) {
	...
	std::complex<double> z_1 = ::exp(std::complex<double>(0, -1 * w));
	std::complex<double> z_2 = ::exp(std::complex<double>(0, -2 * w));

	return (b[0] + b[1] * z_1 + b[2] * z_2) / (std::complex<double>(1) + a[0] * z_1 + a[1] * z_2);
}

void EqFilter::init(EqFilter::FilterType filterType, double f0, double fs, double gain, double Q) {
	this->fs = fs;
	setParameters(filterType, f0, gain, Q);
}

void EqFilter::processSamples(double* output, const double* input, size_t count) {
