...
static int readOHDRHeaderMessageDataspace(struct READER *reader,
	...
	if (ds->dimensionality > MAX_DIMENSIONALITY) {
		log("dimensionality must be low %d\n", MAX_DIMENSIONALITY);
		return MYSOFA_INVALID_FORMAT;
	}
...
static int readOHDRHeaderMessageDataLayout(struct READER *reader,
	...
	case 2:
		...
		data_address = readValue(reader, reader->superblock.size_of_offsets);
		log(" CHUNK %llX, dimensionality %d\n", data_address, dimensionality);
		for (i = 0; i < dimensionality; i++) {
			if((size_t)i < sizeof(data->datalayout_chunk)/sizeof(data->datalayout_chunk[0])) {
				data->datalayout_chunk[i] = readValue(reader, 4);
				log(" %d\n", data->datalayout_chunk[i]);
			} else {
				log(" %llu (out of bounds)\n", readValue(reader, 4));
			}
		}
